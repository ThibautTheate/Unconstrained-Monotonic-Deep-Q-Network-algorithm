...
class NonMonotonicNN(nn.Module):
    '''
    in_d : The total number of inputs
    hidden_layers : a list a the number of neurons, to be used by a network that compresses the non-monotonic variables and by the integrand net.
    nb_steps : Number of integration steps
    n_out : the number of output (each output will be monotonic w.r.t one variable)
    '''
    def __init__(self, in_d, hidden_layers, n_out=1, dev="cpu"):
        super(NonMonotonicNN, self).__init__()
        self.net = []
        hs = [in_d] + hidden_layers + [n_out]
        for h0, h1 in zip(hs, hs[1:]):
            self.net.extend([
    ...
    def forward(self, x, h, only_derivative=False):
        ...
        if only_derivative:
            ...
            for i in range(self.n_out):
                ...
                v[:, i] = 1
                out_d.append(torch.autograd.grad(out, x, v, create_graph=True, retain_graph=True)[0])

            return torch.stack(out_d).squeeze(2).permute(1, 0)
        return out

    '''
    WARNING THIS FUNCTION MAY BE HILL DEFINED AS NOTHING ENFORCE BIJECTIVITY.
    The inverse procedure takes as input y which is the variable for which the inverse must be computed, h are just other conditionning variables.
    One output per n_out.
    y should be a scalar.
    '''
    ...
    def expectation(self, h, x_func, out_deriv, min=-10, max=10, npts=1000):
        ...
        F_values = out_deriv(self.forward(x_values, h_values)).view(b_size, npts, n_out)
        f_values = self.forward(x_values, h_values, True).view(b_size, npts, n_out)
        px = F_values * f_values

        expectation = (px * x_func(x_values).view(b_size, npts, 1).expand(-1, -1, n_out)).sum(1) * dx
